...
@AllArgsConstructor
@ApiModel("过滤地区列表参数包")
public class DistrictsFilterVO {
    @ApiModelProperty(required = false, value = "编号",example = "1101")
    @Range(min = 11, max = 999999, message = "长度不小于2位数字，且不超过6位数字")
    private Integer id;

    @ApiModelProperty(required = false, value = "上级编号",example = "11")
    @Range(min = 0, max = 9999, message = "长度不小于1位数字，且不超过4位数字")
    private Integer pid;
    ...
    @ApiModelProperty(required = false, value = "上级名称",example = "北京")
    @Size(max = 32,message = "长度不超过32个字符")
