...
@AllArgsConstructor
@ApiModel("新增地区视图数据")
public class DistrictsInsertVO {
    @ApiModelProperty(required = true, value = "编号,编辑时必传",example = "1101")
    @NotNull(message = "编号不允许空值")
    @Range(min = 11, max = 999999, message = "长度不小于2位数字，且不超过6位数字")
    private Integer id;

    @ApiModelProperty(required = true, value = "层级,编辑时必传",example = "1")
    @NotNull(message = "层级不允许空值")
    @Range(min = 0, max = 9, message = "长度为1位数字")
    private Integer deep;

    @ApiModelProperty(required = true, value = "名称,编辑时必传",example = "北京")
    @NotBlank(message = "名称不允许空值")
